...
router.patch('/profile/:user_Id', authMiddleware, async (req, res, next) => {
  try {
    ...
    if (!userProfile)
      return res
        .status(404)
        .json({ success: false, message: '프로필 조회에 실패하였습니다.' });

    // 타인의 아이디를 입력했을 때
    if (userProfile.user_Id !== +user_Id) {
      return res
        .status(400)
        .json({ success: false, message: '본인의 프로필을 수정해야합니다.' });
    }
    ...
    return res
      .status(200)
      .json({ message: '프로필 수정이 성공적으로 완료되었습니다.' });
  } catch (err) {
    next(err);
  }
});
...
router.patch('/changePW/:user_Id', authMiddleware, async (req, res, next) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: '비밀번호 수정이 성공적으로 완료되었습니다.' });
  } catch (err) {
    next(err);
  }
});
