...
router.post('/postcomments/:postId', authMiddleware, async (req, res) => {
  try {
    ...
    if (!content) {
      return res
        .status(400)
        .json({ success: false, message: '댓글 내용이 존재하지 않습니다.' });
    }
    ...
    return res
      .status(201)
      .json({ message: '댓글이 성공적으로 작성되었습니다.' });
  } catch (err) {
...
router.patch('/comments', authMiddleware, async (req, res) => {
  try {
    ...
    if (!post_Id) {
      return res
        .status(400)
        .json({ message: '존재하는 게시글 아이디를 작성해주세요.' });
    }
    if (!comment_Id) {
      return res
        .status(400)
        .json({ message: '수정하려는 댓글 아이디를 작성해주세요.' });
    }
...
router.delete('/comments', authMiddleware, async (req, res) => {
  try {
    ...
    if (!comment_Id) {
      return res
        .status(400)
        .json({ message: '삭제하려는 댓글 아이디를 작성해주세요.' });
    }
